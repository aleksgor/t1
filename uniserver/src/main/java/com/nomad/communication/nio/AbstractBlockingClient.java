...
public abstract class AbstractBlockingClient implements Runnable {
     ...
     */
    public AbstractBlockingClient(final InetAddress server, final int port) {
     ...
     */
    public AbstractBlockingClient(final InetAddress server, final int port, final MessageLength messageLength, final int defaultBufferSize) {
        this.server = server;
        this.port = port;
        this.messageLength = messageLength;
        this.defaultBufferSize = defaultBufferSize;
        byteLength = messageLength.byteLength();
    }
        ...
        try {
            ...
            while (state.get() == State.RUNNING) {
                ...
                if (limit>=byteLength) {
                    ...
                    do {
                        final byte[] lengthBytes = new byte[byteLength];
                        System.arraycopy(inBuffer, 0, lengthBytes, 0, byteLength);
                        currentMessageLength = (int) messageLength.bytesToLength(lengthBytes);
                        if (limit >= currentMessageLength) {
                            // enough data to extract the message
                            final byte[] message = new byte[currentMessageLength];
                            System.arraycopy(inBuffer, byteLength, message, 0, currentMessageLength);
                            messageReceived(ByteBuffer.wrap(message));
                            // compact inBuffer
                            final byte[] temp = new byte[inBuffer.length];
                            System.arraycopy(inBuffer, 0, temp, currentMessageLength + byteLength, limit - currentMessageLength - byteLength);
                            inBuffer = temp;
                            limit = limit - currentMessageLength - byteLength;
                        } else if (currentMessageLength > inBuffer.length) {
                            final byte[] temp = new byte[currentMessageLength + byteLength];
                            System.arraycopy(inBuffer, 0, temp, 0, inBuffer.length);
                            inBuffer = temp;
                        }
                    } while (currentMessageLength < limit);
                }
            }
        } catch (final ClosedByInterruptException ie) {
            // do nothing
    ...
    public boolean stop() {
        if (state.compareAndSet(State.RUNNING, State.STOPPING)) {
            try {in.get().close();} catch (final IOException e) {return false;};
            return true;
        }
        return false;
    }
     ...
     */
    public synchronized boolean write(final byte[] buffer) {
        final int length = buffer.length;
        final byte[] lengthBytes = messageLength.lengthToBytes(length);
        try {
            final byte[] outBuffer = new byte[length + byteLength];
            System.arraycopy(lengthBytes, 0, outBuffer, 0, byteLength);
            System.arraycopy(buffer, 0, outBuffer, byteLength, length);
