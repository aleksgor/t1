...
public abstract class AbstractServer implements Runnable {
     ...
     */
    protected AbstractServer(final int port, final MessageLength messageLength, final int defaultBufferSize) {
        this.port = port;
            ...
            while (state.get() == State.RUNNING) {
                ...
                for (final Iterator<SelectionKey> i = selector.selectedKeys().iterator(); i.hasNext();) {
                    ...
                    try {
                        ...
                        if (key.isReadable()) {
                            for (final ByteBuffer message: readIncomingMessage(key)) {
     ...
     */
    protected void resetKey(final SelectionKey key) {
        key.cancel();
     ...
     */
    public void write(final SelectionKey channelKey, final byte[] buffer)  {
        final short length = (short) buffer.length;
        ...
        final ByteBuffer writeBuffer = ByteBuffer.allocate(length + lengthBytes.length);
        ...
        if (buffer!=null && state.get()==State.RUNNING) {
            ...
            try {
                ...
                final SocketChannel channel = (SocketChannel)channelKey.channel();
     ...
     */
    private List<ByteBuffer> readIncomingMessage(final SelectionKey key) throws IOException {
        ...
        ByteBuffer buffer = readMessage(key, readBuffer);
        while (buffer != null) {
            result.add(buffer);
            buffer = readMessage(key, readBuffer);
        }
     ...
     */
    private ByteBuffer readMessage(final SelectionKey key, ByteBuffer readBuffer) {
        int bytesToRead;
        if (readBuffer.remaining()>messageLength.byteLength()) { // must have at least enough bytes to read the size of the message
            final byte[] lengthBytes = new byte[messageLength.byteLength()];
            ...
            if ((readBuffer.limit()-readBuffer.position())<bytesToRead) {
                ...
                if (readBuffer.limit()==readBuffer.capacity()) {
                    ...
                    final int oldCapacity = readBuffer.capacity();
                    final ByteBuffer buffer = ByteBuffer.allocate(bytesToRead + messageLength.byteLength());
        ...
        final byte[] resultMessage = new byte[bytesToRead];
